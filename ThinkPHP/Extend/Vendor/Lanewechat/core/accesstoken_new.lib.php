...
/**
 * 由于订阅号无法调用oAuthor接口，需要借助第三方，所以需要一个新的全局token
...
class AccessToken_New{
    ...
    private static function _getAccessToken(){
        $url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='.WECHAT_APPID_AUTHOR.'&secret='.WECHAT_APPSECRET_AUTHOR;
...
//        fclose($f);
        ...
        $redis->setWechatToken($accessTokenJson,'youshucc_token');
        return $accessToken;
    }
...
//        $data = file_get_contents('access_token');
        $redis = new RedisService();
        $data = $redis->getWechatToken('youshucc_token');
