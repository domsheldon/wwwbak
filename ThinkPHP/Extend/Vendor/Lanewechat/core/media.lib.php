...
//        $data['media'] = Curl::addFile($filename);
    ...
    /**
     * 上传永久素材，罗勤勇 2016-1-13
     * @param $filename
     * @param $type
     * @return bool|mixed
     */
    public static function upload_material($filename, $type){
        //获取ACCESS_TOKEN
        $accessToken = AccessToken::getAccessToken();
        $queryUrl = 'http://api.weixin.qq.com/cgi-bin/material/add_material?access_token='.$accessToken.'&type='.$type;
        $data = array();
//        $data['media'] = Curl::addFile($filename);
        //php5.5以下，直接上传@的文件路径
        $data['media'] = $filename;
