					...
					<form action="" method="get">
						...
						<table cellpadding="0" cellspacing="0" width="100%" class="sortable">
							...
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?><tr>
									...
									<td><?php echo ($info["feed_back"]); ?></td>
									<td><?php echo ($info["phone_info"]); ?></td>
									<td><?php echo (date("m-d H:i:s",$info["create_time"])); ?></td>
									<td><a href="__GROUP__/Users/backPush?uid=<?php echo ($info["user_id"]); ?>&uname=<?php echo ($info["name"]); ?>">回复</a></td>
						...
						<div class="pagination right">
							<a href="__GROUP__/Back?page=<?php echo ($page-1); ?>">&laquo;</a>
							<?php $__FOR_START_7907__=1;$__FOR_END_7907__=$sum+1;for($i=$__FOR_START_7907__;$i < $__FOR_END_7907__;$i+=1){ if(($page) == $i): ?><a href="__GROUP__/Back?page=<?php echo ($i); ?>" class="active"><?php echo ($i); ?></a>
							<?php else: ?>
								<a href="__GROUP__/Back?page=<?php echo ($i); ?>"><?php echo ($i); ?></a><?php endif; } ?>
							<a href="__GROUP__/Back?page=<?php echo ($page+1); ?>">&raquo;</a>
						</div>		<!-- .pagination ends -->
