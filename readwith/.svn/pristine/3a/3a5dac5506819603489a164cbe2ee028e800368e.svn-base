...
class RedisModel extends Model{
	...
	* @date 2016-2-29
	*/
	public function setIncNoteDigg($note_id) {
		$this->redis->zincrby(self::NOTEDIGG, 1, $note_id);
		$this->upNoteCount($note_id, 1 ,'digg_count');
	}
	/**
	* 赞-1
	* @param $note_id 
	* @date 2016-2-29
	*/
	public function setDecNoteDigg($note_id) {
		$this->redis->zincrby(self::NOTEDIGG, '-1', $note_id);
		$this->upNoteCount($note_id, -1 ,'digg_count');
	}
	/**
	* 对hash存储的note更新digg_count/comment_count
	* @param $note_id 
	* @param $step	
	* @param $field 字段digg_count/comment_count
	* @date 2016-3-1
	*/
	public function upNoteCount($note_id, $step='1', $field='comment_count') {
		//更新哈希的digg_count值
		$note_json=$this->redis->hget(self::NOTE, $note_id);
		$note=json_decode($note_json, true);
		$note[$field]=$note[$field] + $step;
		$this->redis->hset(self::NOTE, $note_id, json_encode($note));
	}
	/**
	* 累加到此用户下的note_id
	* @param $user_id 
...
//		$this->setNoteAllRedis();
	...
	*/
	public function setNoteAllRedis() {
		$list=M('Note')->where("id > 5670 and is_delete=0")->select();
		foreach ($list as $l){
			$this->redis->hset(self::NOTE, $l['id'], json_encode($l));
			$this->setNoteTimeRedis($l['create_time'], $l['id']);		//时间排序
			$this->setNoteDiggRedis($l['digg_count'], $l['id']);		//点赞排序
	...
	*/
	public function setCheckData($pack_id) {
		...
		$check_list=$pack->packDayCheck($pack_id);
	...
	*/
     ...
     * @param $msg_type $包括note,comment,digg,undigg取消,check
     */
