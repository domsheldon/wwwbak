...
class UploadFile {//类定义开始
            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    if(!empty($thumbFile[$i])) {
                        ...
                        $prename	=	str_replace("_", "", $prefix);
                    }
                    //根据前缀放在不同的目录下
                   	Image::image_resize($filename, $thumbPath.$prename."/".$thumbname.'.'.$thumbExt, $thumbWidth[$i],$thumbHeight[$i]);
                }
    ...
    private function getSaveName($filename) {
        ...
        //这里如果数据库中有头像，更改头像时
    	if($_COOKIE['avatar']){
        	$saveName=substr($_COOKIE['avatar'],9);
        }
        if($this->autoSub) {
            // 使用子目录保存文件
