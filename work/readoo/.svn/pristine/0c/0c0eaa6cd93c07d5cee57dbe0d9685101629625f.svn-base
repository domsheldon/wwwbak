		...
		<div class="wrapper">		<!-- wrapper begins -->
			...
			<div class="block">
				<div class="block_head">
					...
					<?php if(($list_type) != "search"): ?><h2>活动列表</h2>
					<?php else: ?>
						<h2>搜索结果</h2><?php endif; ?>
					<form action='__GROUP__/Ad/ad' method='get'>
					<input type='submit' class='submit small' value='新增广告'/>
					</form>
				</div>		<!-- .block_head ends -->
				
				<div class="block_content">
					<form action="__GROUP__/Ad/del" method="get">
						...
						<table cellpadding="0" cellspacing="0" width="100%" class="sortable">
							...
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?><tr>
									...
									<td><?php echo ($info["ad_desc"]); ?></td>
									<td>
									<?php if(($info["is_off"]) == "1"): ?>已关闭<?php else: ?>开启<?php endif; ?></td>
									<td>
									<a href="__GROUP__/Ad/baomingList?id=<?php echo ($info["id"]); ?>">查看报名信息</a> | <a href="__GROUP__/Ad/ad?id=<?php echo ($info["id"]); ?>">编辑</a></td>
